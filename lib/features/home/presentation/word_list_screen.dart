...
class _WordListScreenState extends State<WordListScreen> {
  ...
  Future<void> _fetchData() async {
    try {
      final res = await widget.repository.getWords(categoryId: widget.categoryId, pos: widget.pos, isFavorite: widget.isFavorite);
      setState(() { words = res['data']; isLoading = false; });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text(widget.title, style: const TextStyle(color: Color(0xFF00AA5B), fontWeight: FontWeight.bold)), backgroundColor: Colors.white, iconTheme: const IconThemeData(color: Color(0xFF00AA5B))),
      body: isLoading
            ...
            child: Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(
                        word['is_favorite'] == true ? Icons.bookmark : Icons.bookmark_border,
                        color: word['is_favorite'] == true ? const Color(0xFF00AA5B) : Colors.grey
                    ),
